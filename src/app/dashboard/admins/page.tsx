...
export default function AdminsPage() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const response = await adminApiClient.get('/admins', { params });

      if (response.data?.success) {
        const responseData: AdminsResponse = response.data.data;
      ...
      setError(error.response?.data?.message || error.message || 'Failed to load admins');
      setAdmins([]);
      setPagination(null);
    } finally {
      setIsLoading(false);
  ...
  const handleCreateAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await adminApiClient.post('/admins', formData);
  ...
  const handleEditAdmin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await adminApiClient.put(`/admins/${editingAdmin.id}`, formData);
  ...
  const handleDeleteAdmin = async (adminId: string) => {
    ...
    try {
      await adminApiClient.delete(`/admins/${adminId}`);
      fetchAdmins();
    } catch (error: any) {
  ...
  const handleToggleActive = async (adminId: string, isActive: boolean) => {
    try {
      await adminApiClient.put(`/admins/${adminId}`, { isActive: !isActive });
  ...
  const handleResetPassword = async (adminId: string) => {
    if (!confirm('Are you sure you want to reset this admin\'s password? They will receive an email with reset instructions.')) {
      return;
    }

    try {
      await adminApiClient.post(`/admins/${adminId}/reset-password`);
  ...
  if (isLoading && admins.length === 0) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <p className="ml-4 text-gray-600">Loading admins...</p>
      </div>
    );
  }
        ...
        <div className="px-4 py-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg leading-6 font-medium text-gray-900">
              Admin Users ({pagination?.total || 0})
            </h3>
            {isLoading && (
                    ...
                    <tr key={admin.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          >
                            {admin.isActive ? <XCircleIcon className="h-4 w-4" /> : <CheckCircleIcon className="h-4 w-4" />}
                          </button>
                        </div>
                      </td>
                    </tr>
